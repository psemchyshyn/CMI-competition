...
@click.option("--dir", "-d", type=Path, default="/mnt/data/psemchyshyn/sleep-detection/results/output/train")
@click.option("--extentions", "-e", type=list[str], default=["best_model.pth", ".hydra/*.yaml"])
@click.option("--user_name", "-u", default="pavlosemchyshyn")
@click.option("--new", "-n", is_flag=True)
def main(
    title: str,
    dir: Path,
    extentions: list[str] = [".pth", ".yaml"],
    user_name: str = "pavlosemchyshyn",
    new: bool = False,
):
    ...
    """
    tmp_dir = Path("./tmp/new")
    tmp_dir_outer = Path("./tmp")
    ...
    with open(tmp_dir_outer / "dataset-metadata.json", "w") as f:
